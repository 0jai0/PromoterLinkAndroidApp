...
import { View, ScrollView, Text, ActivityIndicator, TouchableOpacity, RefreshControl, Modal, Dimensions } from "react-native";
...
const HomeScreen = ({navigation}) => {
  ...
  const { users, isLoading, error, page, totalPages, goToPage, refetch, hasFetched } = useUsers(filters);
  ...
  const hasActiveFilters = searchTerm || selectedAdCategories.length > 0 || 
    selectedPlatforms.length > 0 || selectedAudienceTypes.length > 0 || 
    selectedLocations.length > 0 || minFollowers || maxFollowers;
    ...
    return (
      <View className="flex-1 bg-slate-50 justify-center items-center">
        <View className="items-center">
          ...
          <Text className="text-slate-500 mt-4 text-base">Checking authentication...</Text>
        </View>
      </View>
    );
  }
    ...
    return (
      <View className="flex-1 bg-slate-50 justify-center items-center p-8">
        <View className="bg-white p-8 rounded-2xl w-full max-w-md shadow-sm border border-slate-200">
          ...
          <Text className="text-slate-800 text-xl font-bold mb-4 text-center">Authentication Required</Text>
          ...
          >
            ...
            <Text className="text-white font-semibold ml-2 text-base">Go to Login</Text>
          </TouchableOpacity>
        </View>
      </View>
    );
        ...
        <View className="pt-5 p-4 bg-slate-50 flex-1">
            ...
            {hasActiveFilters && (
              <View className="flex-row items-center justify-between mt-4 p-3 bg-blue-50 rounded-lg border border-blue-100">
                <View className="flex-row items-center">
                  ...
                  <Text className="text-slate-700 ml-2 text-sm font-medium">Filters active</Text>
                </View>
                <TouchableOpacity 
                ...
                >
                  ...
                  <Text className="text-slate-700 text-xs ml-1 font-medium">Clear all</Text>
                </TouchableOpacity>
              </View>
            )}
          </View>
          ...
          {error && (
            <View className="bg-red-50 p-4 rounded-xl mb-4 border border-red-100">
              <View className="flex-row items-center mb-2">
                ...
                <Text className="text-red-800 ml-2 font-medium">Error Loading Data</Text>
              </View>
          ...
          <View className="flex-row justify-between items-center mb-4">
            <Text className="text-slate-800 text-lg font-semibold">
              {isLoading ? "Searching..." : users.length > 0 ? "Influencers" : "No Results"}
            </Text>
            {users.length > 0 && (
          ...
          >
            {isLoading && !hasFetched ? (
              <View className="flex-1 justify-center items-center py-12">
                <ActivityIndicator size="large" color="#3B82F6" />
                <Text className="text-slate-500 mt-4 text-base">Loading influencers...</Text>
              </View>
              ...
              <View className="py-12 items-center bg-white rounded-xl p-6 mt-2 border border-slate-200 shadow-xs">
                ...
                <Text className="text-slate-800 mt-4 text-lg font-medium">No influencers found</Text>
                ...
                {hasActiveFilters && (
                  ...
                  >
                    ...
                    <Text className="text-white ml-2 font-medium">Clear all filters</Text>
                  </TouchableOpacity>
                )}
              </View>
            ) : (
                ...
                <View className="space-y-3 mb-4">
                  {users.map((userItem) => (
                    <UserCard
                      ...
                      onAddToList={(targetUser) => handleAddToList(
                        targetUser, 
                        user, 
                        contacts, 
                        setAlert, 
                      ...
                      onChatNow={(targetUser) => handleChatNow(
                        targetUser, 
                        user, 
                        contacts, 
                        navigation, 
